...
The search script lives inside this skill's own directory. This skill is installed project-scoped, so invoke it by its path relative to the project root (the paths below assume that working directory); use the equivalent absolute path if running from elsewhere:
